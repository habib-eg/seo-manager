                    ...
                    <div class="card">
                        <div v-if="loaded" class="card-body text-center">
                            <table v-show="routes.length > 0" class="routes-table table text-left">
                                ...
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col">{{$t('main.url')}}</th>
                                    <th scope="col">{{$t('main.params')}}</th>
                                    <th scope="col">{{$t('main.mapping')}}</th>
                                    <th scope="col">{{$t('main.keywords')}}</th>
                                    <th scope="col">{{$t('main.description')}}</th>
                                    <th scope="col">{{$t('main.title')}}</th>
                                    <th scope="col">{{$t('main.url')}}</th>
                                    <th scope="col">{{$t('main.author')}}</th>
                                    <th scope="col">{{$t('main.title_dynamic')}}</th>
                                    <th scope="col">{{$t('main.open_graph_data')}}</th>
                                    <th scope="col">{{$t('main.actions')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                            ...
                            <div v-show="routes.length === 0" class="no-routes">
                                <!-- Image -->
                                <img :src="path+'/vendor/lionix/img/scale.svg'" alt="..." class="img-fluid"
                                     style="max-width: 182px;">
                                <!-- Title -->
                                <h1>
                                    {{$t('main.no_routes_yet')}}
                                </h1>
                                <!-- Subtitle -->
                                <p class="text-muted">
                                    {{$t('main.import_your_all_routes_by_clicking_button_below')}}
                                </p>
                                <!-- Button -->
                                <button @click="importRoutes" :clas="{'is-loading': importing}"
                                        class="import-routes btn btn-primary">
                                    {{$t('main.import_routes')}}
                                </button>
                            </div>
                        </div>
